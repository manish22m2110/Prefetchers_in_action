...
output_file="L1DissPerKiloInstructions.txt"
> "$output_file"
...
for trace_file in *.txt; do
  if [ -f "$trace_file" ]; then
    ...
    l2c_total_access=$(grep -oP 'cpu0_L1D TOTAL\s+ACCESS:\s+\K\d+' "$trace_file")
    l2c_miss=$(grep -oP 'cpu0_L1D TOTAL\s+ACCESS:\s+\d+\s+HIT:\s+\d+\s+MISS:\s+\K\d+' "$trace_file")
...
echo "L1D Miss Per Kilo Instructions calculations completed. Results saved in $output_file"
